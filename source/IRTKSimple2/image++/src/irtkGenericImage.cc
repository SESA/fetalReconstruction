...
irtkGenericImage<VoxelType>::irtkGenericImage(void) : irtkBaseImage() {
  _attr._x = 0;
  _attr._y = 0;
  _attr._z = 0;
  _attr._t = 0;
...
template <class VoxelType>
irtkGenericImage<VoxelType>::irtkGenericImage(int x, int y, int z, int t)
    : irtkBaseImage() {
...
template <class VoxelType>
irtkGenericImage<VoxelType>::irtkGenericImage(char *filename) {
  // Initialize data
  _matrix = NULL;
...
template <class VoxelType>
irtkGenericImage<VoxelType>::irtkGenericImage(const irtkImageAttributes &attr)
    : irtkBaseImage() {
    // Initialize data
    _matrix = NULL;
...
irtkGenericImage<VoxelType>::irtkGenericImage(
    const irtkGenericImage<VoxelType2> &image) {
  int i, n;
  VoxelType *ptr1;
  VoxelType2 *ptr2;
...
template <class VoxelType>
void irtkGenericImage<VoxelType>::Initialize(const irtkImageAttributes &attr) {
  // Free memory
...
template <class VoxelType>
void irtkGenericImage<VoxelType>::Write(const char *filename) {
  // Allocate file reader
...
template <class VoxelType>
VoxelType irtkGenericImage<VoxelType>::GetAverage(int toggle) const {
  float average = 0;
  int i, n, m;
  VoxelType *ptr;
...
template <class VoxelType>
VoxelType irtkGenericImage<VoxelType>::GetSD(int toggle) const {
  // Initialize pixels
...
void irtkGenericImage<VoxelType>::GravityCenter(irtkPoint& p, int ds,
  ...
  // irtkInterpolateImageFunction::New(Interpolation_CSpline,
  // (irtkBaseImage*)this);
...
template <class VoxelType>
void irtkGenericImage<VoxelType>::Saturate(double q0, double q1) {
  if ((q0 == 0) && (q1 == 1))
    return;
  if ((q0 < 0) || (q1 > 1)) {
    stringstream msg;
...
irtkGenericImage<VoxelType>::GetRegion(int k, int m) const {
  int i, j;
...
template <class VoxelType>
irtkGenericImage<VoxelType> irtkGenericImage<VoxelType>::GetFrame(int l) const {
  int i, j, k;
...
operator=(const irtkGenericImage<VoxelType> &image) {
  int i, n;
  VoxelType *ptr1, *ptr2;
...
template <class VoxelType2>
irtkGenericImage<VoxelType> &irtkGenericImage<VoxelType>::
operator=(const irtkGenericImage<VoxelType2> &image) {
  int i, n;
  VoxelType *ptr1;
  VoxelType2 *ptr2;
...
template <class VoxelType>
bool irtkGenericImage<VoxelType>::
operator==(const irtkGenericImage<VoxelType> &image) {
  int i, n;
  VoxelType *ptr1, *ptr2;
...
template <class Type>
void irtkGenericImage<Type>::ImageToVTK(vtkStructuredPoints *vtk) {
  int i, n;
  double x, y, z;
  Type *ptr1, *ptr2;
